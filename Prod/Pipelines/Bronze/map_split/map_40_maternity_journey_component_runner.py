...
_MAP_SPLIT_GROUP = 'map_40_maternity_journey'
_MAP_SPLIT_ALLOWED_COMPONENTS = ['map_mat_pregnancy', 'map_mat_birth', 'map_mat_vte_assessment', 'map_family_history', 'map_patient_journey', 'map_implant_details']
...
if _map_split_component_selected('map_mat_pregnancy'):
...
if _map_split_component_selected('map_mat_birth'):
...
if _map_split_component_selected('map_mat_vte_assessment'):
...
if _map_split_component_selected('map_family_history'):
...
if _map_split_component_selected('map_patient_journey'):
...
def run_map_patient_journey_legacy_reconciliation(config: MapPatientJourneyConfig=MAP_PATIENT_JOURNEY_CONFIG, legacy_table: Optional[str]=None) -> Dict[str, object]:
    """Reconcile the target to the complete pinned location-history source scope.

    The target contract deliberately retains inactive rows and every Trust value.
    Filtering the validator to active Barts encounters contradicts the published
    schema and full-build path, and falsely labels those retained rows as stale.
    """
    if not _mpj_table_exists(config.target_table):
    ...
    expected = (
        ...
        .select(
            F.col('ENCNTR_LOC_HIST_ID')
            .cast('long')
            .alias('ENCNTR_LOC_HIST_ID')
        )
        .dropDuplicates()
    )
    actual = (
    ...
    result = {
        'scope': 'all_pinned_location_history_rows',
        'expected_source_keys': int(expected.count()),
...
if _map_split_component_selected('map_implant_details'):
