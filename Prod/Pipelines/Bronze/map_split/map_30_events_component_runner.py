...
import json as _map_split_json
import time as _map_split_time
_MAP_SPLIT_GROUP = 'map_30_events'
_MAP_SPLIT_ALLOWED_COMPONENTS = ['map_numeric_events', 'map_text_events', 'map_date_events', 'map_nomen_events', 'map_coded_events']
_MAP_SPLIT_COMPONENT_TARGETS = {'map_address': ['4_prod.bronze.map_address', '4_prod.bronze.map_address_epc'], 'map_person': ['4_prod.bronze.map_person'], 'map_care_site': ['4_prod.bronze.map_care_site'], 'map_medical_personnel': ['4_prod.bronze.map_medical_personnel'], 'map_encounter': ['4_prod.bronze.map_encounter'], 'map_diagnosis': ['4_prod.bronze.map_diagnosis'], 'map_problem': ['4_prod.bronze.map_problem'], 'map_med_admin': ['4_prod.bronze.map_med_admin'], 'map_procedure': ['4_prod.bronze.map_procedure'], 'map_death': ['4_prod.bronze.map_death'], 'map_numeric_events': ['4_prod.bronze.map_numeric_events'], 'map_text_events': ['4_prod.bronze.map_text_events'], 'map_date_events': ['4_prod.bronze.map_date_events'], 'map_nomen_events': ['4_prod.bronze.map_nomen_events'], 'map_coded_events': ['4_prod.bronze.map_coded_events'], 'map_mat_pregnancy': ['4_prod.bronze.map_mat_pregnancy'], 'map_mat_birth': ['4_prod.bronze.map_mat_birth'], 'map_mat_vte_assessment': ['4_prod.bronze.map_mat_vte_assessment'], 'map_family_history': ['4_prod.bronze.map_family_history'], 'map_patient_journey': ['4_prod.bronze.map_patient_journey'], 'map_implant_details': ['4_prod.bronze.map_implant_details'], 'map_pathology': ['4_prod.bronze.map_pathology']}
_MAP_SPLIT_SELECTED_COMPONENT = str(dbutils.widgets.get('map_component')).strip()
if _MAP_SPLIT_SELECTED_COMPONENT not in _MAP_SPLIT_ALLOWED_COMPONENTS:
    raise ValueError(
        f'map_component must be one of {_MAP_SPLIT_ALLOWED_COMPONENTS}; '
        f'got {_MAP_SPLIT_SELECTED_COMPONENT!r}'
    )
...
if not _map_split_manifest_raw or not _map_split_manifest_hash:
    raise ValueError('Map split component is missing the initializer-pinned source manifest')
_map_split_manifest = {
    str(key): int(value)
    for key, value in _map_split_json.loads(_map_split_manifest_raw).items()
}
_map_split_computed_hash = _map_split_hashlib.sha256(
    _map_split_json.dumps(_map_split_manifest, sort_keys=True).encode('utf-8')
).hexdigest()[:16]
if _map_split_computed_hash != _map_split_manifest_hash:
    raise ValueError({
        'message': 'Map split source manifest hash mismatch',
...
_MAP_SPLIT_MODE = True
_PIPELINE_ACTIVE_CANONICAL_TARGETS = list(
    _MAP_SPLIT_COMPONENT_TARGETS[_MAP_SPLIT_SELECTED_COMPONENT]
)
_PIPELINE_SKIP_TARGETS = {
    table_name.rsplit('.', 1)[-1]
    for table_name in _PIPELINE_CANONICAL_TARGETS
    if table_name not in _PIPELINE_ACTIVE_CANONICAL_TARGETS
}
_PIPELINE_FORCE_FULL_REFRESH = _map_split_effective_full_refresh == 'true'
_PIPELINE_FULL_REFRESH = (
    _PIPELINE_FORCE_FULL_REFRESH
    or any(not table_exists(table_name) for table_name in _PIPELINE_ACTIVE_CANONICAL_TARGETS)
)
def _map_split_component_selected(component_name):
    return component_name == _MAP_SPLIT_SELECTED_COMPONENT
print(
    f'[MAP-SPLIT] group={_MAP_SPLIT_GROUP} component={_MAP_SPLIT_SELECTED_COMPONENT} '
    f'full_refresh={_PIPELINE_FULL_REFRESH} run_id={_PIPELINE_RUN_ID} '
    f'manifest={_PIPELINE_SOURCE_MANIFEST_HASH}'
)
...
if _map_split_component_selected('map_numeric_events'):
...
if _map_split_component_selected('map_text_events'):
...
if _map_split_component_selected('map_date_events'):
...
_map_split_pending_before = len(_PIPELINE_PENDING_CHECKPOINTS)
for _map_split_attempt in range(1, 7):
    _pipeline_commit_all_checkpoints()
    if not _PIPELINE_PENDING_CHECKPOINTS:
        break
    _map_split_time.sleep(min(2 ** _map_split_attempt, 30))
if _PIPELINE_PENDING_CHECKPOINTS:
    raise RuntimeError(
        'Map component data succeeded but source checkpoints could not be committed: '
        + repr(sorted(_PIPELINE_PENDING_CHECKPOINTS))
    )
_map_split_states = {}
for _map_split_target in _MAP_SPLIT_COMPONENT_TARGETS[_MAP_SPLIT_SELECTED_COMPONENT]:
    _map_split_state = _pipeline_target_state(_map_split_target)
    if not _map_split_state:
        raise RuntimeError(f'Missing completion state for {_map_split_target}')
    if _map_split_state.get('component') != _MAP_SPLIT_SELECTED_COMPONENT:
        raise RuntimeError({_map_split_target: _map_split_state})
    if _map_split_state.get('run_id') != _PIPELINE_RUN_ID:
        raise RuntimeError({_map_split_target: _map_split_state, 'expected_run_id': _PIPELINE_RUN_ID})
    _map_split_states[_map_split_target] = _map_split_state
_pipeline_audit(None, 'COMPONENT_TASK_SUCCESS', {
    'component': _MAP_SPLIT_SELECTED_COMPONENT,
    'group': _MAP_SPLIT_GROUP,
    'checkpoint_count': _map_split_pending_before,
    'target_states': _map_split_states,
})
_map_split_result = {
    'status': 'SUCCESS',
    ...
    'targets': _MAP_SPLIT_COMPONENT_TARGETS[_MAP_SPLIT_SELECTED_COMPONENT],
    'checkpoint_count': _map_split_pending_before,
}
...
# (whenMatchedUpdate(set=retained_updates)), so the MERGE runs only when the target has
# no anonymous state at all and the sidecar exists. Installed by Anon__b1_bronze_job_install.
import json as _anon_json
from pyspark.sql import functions as _anon_F

if "_MAP_SPLIT_SELECTED_COMPONENT" in globals() and _MAP_SPLIT_SELECTED_COMPONENT == "map_text_events":
    _anon_target = "4_prod.bronze.map_text_events"
    _anon_state = "6_mgmt.anon.state_text_event"
    _anon_keys = ["EVENT_ID"]
    _anon_outputs = ["anon_text_result"]
    _ANON_STATE_TYPES = {
        "anon_status": "STRING",
    ...
    _anon_columns = {field.name for field in spark.table(_anon_target).schema.fields}
    for _anon_name in _anon_outputs:
        if _anon_name not in _anon_columns:
            spark.sql(
                f"ALTER TABLE {_anon_qtable(_anon_target)} "
                f"ADD COLUMNS (`{_anon_name}` STRING)"
            )
    for _anon_name, _anon_type in _ANON_STATE_TYPES.items():
        if _anon_name not in _anon_columns:
            spark.sql(
                f"ALTER TABLE {_anon_qtable(_anon_target)} "
                f"ADD COLUMNS (`{_anon_name}` {_anon_type})"
            )
    if _anon_outputs[0] not in _anon_columns or "anon_status" not in _anon_columns:
        for _anon_name in _anon_outputs:
            spark.sql(
    ...
    _anon_needs_reattach = (
        spark.catalog.tableExists(_anon_state)
        and spark.table(_anon_target).where(_anon_F.col("anon_status").isNotNull()).limit(1).count() == 0
    )
    if _anon_needs_reattach:
        _anon_join = " AND ".join(
            f"t.`{column}` <=> s.`{column}`" for column in _anon_keys
        )
        _anon_set = ", ".join(
            f"t.`{column}` = s.`{column}`"
            for column in _anon_outputs + list(_ANON_STATE_TYPES)
        )
        _anon_expected = spark.table(_anon_target).alias("t").join(
            spark.table(_anon_state).alias("s"),
            _anon_F.col("t.EVENT_ID").eqNullSafe(_anon_F.col("s.EVENT_ID")),
            "inner",
        ).count()
        spark.sql(
            f"MERGE INTO {_anon_qtable(_anon_target)} t "
            f"USING {_anon_qtable(_anon_state)} s ON {_anon_join} "
            f"WHEN MATCHED THEN UPDATE SET {_anon_set}"
        )
        _anon_restored = spark.table(_anon_target).where(
            _anon_F.col("anon_status").isNotNull()
        ).count()
        if _anon_restored != _anon_expected:
            raise AssertionError(
                f"anonymous-state reattach for {_anon_target}: "
                f"expected={_anon_expected}, restored={_anon_restored}"
            )
        print(_anon_json.dumps({
            "event": "anon_text_event_state_reattached",
            "rows": _anon_restored,
        }))
    else:
        print(_anon_json.dumps({
            "event": "anon_text_event_state_reattach_skipped",
            "state_exists": spark.catalog.tableExists(_anon_state),
        }))
# END_ANON_TEXT_STATE_REATTACH_V3_3_TEXT_EVENT

dbutils.notebook.exit(_map_split_json.dumps(_map_split_result, sort_keys=True, default=str))
