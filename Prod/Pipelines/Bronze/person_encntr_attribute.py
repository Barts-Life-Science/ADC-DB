...
# release: bronze_completeness_20260822_a7_v2 — orphan-aware encounter linkage gate
import json
...
def gate_target(name, attrs, link_floor):
    ...
    assert link_rate>=link_floor, f"{name} G4 link rate {link_rate:.5f}"

    synchronized_link_rate = None
    raw_present_unlinked_entity_ids = None
    source_orphan_entity_ids = None
    if name == "map_encounter_attribute":
        ...
        unlinked_ids=(d.where("LINK_STATUS<>'LINKED'")
                      .select(F.col("ENTITY_ID").cast("bigint").alias("ENTITY_ID"))
                      .where(F.col("ENTITY_ID").isNotNull()).distinct())
        raw_present_unlinked_entity_ids=unlinked_ids.join(raw_ids,"ENTITY_ID","inner").count()
        source_orphan_entity_ids=unlinked_ids.join(raw_ids,"ENTITY_ID","left_anti").count()
        assert raw_present_unlinked_entity_ids==0, (
            f"{name} G4 map omission: {raw_present_unlinked_entity_ids} unlinked "
            "ENCNTR_ID values are present in raw mill_encounter"
        )
        ...
        assert synchronized_link_rate >= link_floor, (
            f"{name} G4 synchronized link rate {synchronized_link_rate:.5f} "
            f"below source-orphan tolerance {link_floor:.5f} at "
            f"map_encounter horizon {link_horizon}"
        )
    ...
    metrics={"target":target,"rows":total,"linked":linked,"link_rate":link_rate,
             "synchronized_link_rate":synchronized_link_rate,"active":active,
             "raw_present_unlinked_entity_ids":raw_present_unlinked_entity_ids,
             "source_orphan_entity_ids":source_orphan_entity_ids}
    print(metrics)
    return metrics

def run_gates():
    gate_target("map_person_attribute",PERSON_ATTRS,0.989)
    gate_target("map_encounter_attribute",ENCNTR_ATTRS,0.997)
    print("A7 gates PASS")
