...
_PIPELINE_CHECKPOINT_TABLE = "6_mgmt.bronze.map_pipeline_source_checkpoints"
_PIPELINE_AUDIT_TABLE = "6_mgmt.bronze.map_pipeline_audit"
...
def _pipeline_hardening_sql_identifier(table_name: str) -> str:
    return '.'.join('`' + part.replace('`', '``') + '`' for part in table_name.split('.'))

def _pipeline_hardening_snapshot_fallback_allowed(exc: BaseException) -> bool:
    messages = []
    current = exc
...
def _pipeline_hardening_read_pinned_snapshot(table_name: str, version: int):
    ...
    try:
        ...
        if not _pipeline_hardening_snapshot_fallback_allowed(exc):
            raise
        row = (
            spark.sql('DESCRIBE HISTORY ' + _pipeline_hardening_sql_identifier(table_name) + ' LIMIT 1')
            .select('version')
            .first()
        )
...
def get_incremental_data_with_cdf(
    ...
    if not table_exists(target_table):
        if ending_version is not None:
            incremental_df = _pipeline_hardening_read_pinned_snapshot(
                source_table,
                ending_version,
            )
        ...
        try:
            ...
            if "_change_type" in incremental_df.columns:
                # Exercise the lazy CDF relation while this recovery block is active.
                incremental_df.limit(1).collect()
                incremental_df = (
                    incremental_df
                    .where(
...
_PIPELINE_POST_DEPLOYMENT_ASSERTIONS = [
    ...
    {
        # PERSON rows intentionally retain full address history. ORGANIZATION rows
        # are retained only while active and currently effective.
        "name": "address_scope_contract",
        "tables": ["{prefix}.map_address{suffix}"],
        "sql": (
            "SELECT count(*) AS violations FROM "
            "{prefix}.map_address{suffix} "
            "WHERE upper(trim(coalesce(PARENT_ENTITY_NAME, ''))) "
            "NOT IN ('PERSON', 'ORGANIZATION') "
            "OR (upper(trim(coalesce(PARENT_ENTITY_NAME, ''))) = 'ORGANIZATION' "
            "AND (coalesce(ACTIVE_IND, 0) <> 1 "
            "OR NOT (END_EFFECTIVE_DT_TM IS NULL "
            "OR END_EFFECTIVE_DT_TM > current_timestamp())))"
        ),
    },
    {
        # Person postcodes publish the outward code. Outward codes can be four
        # characters, so a fixed three-character ceiling is not a valid mask test.
        "name": "address_person_mask_matches_outward",
        "tables": ["{prefix}.map_address{suffix}"],
        "sql": (
            "SELECT count(*) AS violations FROM "
            "{prefix}.map_address{suffix} "
            "WHERE upper(trim(coalesce(PARENT_ENTITY_NAME, ''))) = 'PERSON' "
            "AND NOT (masked_zipcode <=> POSTCODE_OUTWARD)"
        ),
    },
    {
        # Organisations retain their source postcode. Detect the exact person-mask
        # truncation signature without rejecting genuinely short source values.
        "name": "address_organisation_not_masked",
        "tables": ["{prefix}.map_address{suffix}"],
        "sql": (
            "SELECT count(*) AS violations FROM "
            "{prefix}.map_address{suffix} "
            "WHERE upper(trim(coalesce(PARENT_ENTITY_NAME, ''))) = 'ORGANIZATION' "
            "AND masked_zipcode IS NOT NULL "
            "AND SOURCE_ZIPCODE IS NOT NULL "
            "AND length(trim(SOURCE_ZIPCODE)) > 3 "
            "AND masked_zipcode = substr(trim(SOURCE_ZIPCODE), 1, 3)"
        ),
    },
    {
        # Dotted ICD-10 codes are valid canonical values. The invariant is the
        # published one-row-per-diagnosis grain.
        "name": "diagnosis_key_integrity",
        "tables": ["{prefix}.map_diagnosis{suffix}"],
        "sql": (
            "SELECT count(*) AS violations FROM ("
            "SELECT DIAGNOSIS_ID FROM {prefix}.map_diagnosis{suffix} "
            "GROUP BY DIAGNOSIS_ID "
            "HAVING DIAGNOSIS_ID IS NULL OR count(*) <> 1)"
        ),
    },
    {
        # Service assignments are optional source facts; null values are retained.
        # The durable invariant is one non-null row per personnel identifier.
        "name": "medical_personnel_key_integrity",
        "tables": ["{prefix}.map_medical_personnel{suffix}"],
        "sql": (
            "SELECT count(*) AS violations FROM ("
            "SELECT PERSON_ID FROM {prefix}.map_medical_personnel{suffix} "
            "GROUP BY PERSON_ID "
            "HAVING PERSON_ID IS NULL OR count(*) <> 1)"
        ),
    },
    {
        # BirthRow_ID is the stable retained source-row key. The old assertion
        # compared the single-layer table to itself and could never fail.
        "name": "mat_birth_key_integrity",
        "tables": ["{prefix}.map_mat_birth{suffix}"],
        "sql": (
            "SELECT count(*) AS violations FROM ("
            "SELECT BirthRow_ID FROM {prefix}.map_mat_birth{suffix} "
            "GROUP BY BirthRow_ID "
            "HAVING BirthRow_ID IS NULL OR count(*) <> 1)"
        ),
    },
    {
    ...
    },
    {
        ...
        "name": "person_birth_parts_disagree",
        "tables": ["{prefix}.map_person{suffix}"],
        "sql": (
            "SELECT count(*) AS violations FROM "
            "{prefix}.map_person{suffix} "
