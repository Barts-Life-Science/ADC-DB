...
# S5-POWERFORM BEGIN
S5_MAPPING_VERSION="20260813_s5_v1"

def apply_s5_powerform_precedence(df):
    """Fenced replacement for final concept selection after canonical and PowerForm joins."""
    return (df
      .withColumn("QUESTION_MAPPING_CONFLICT_IND",
        F.col("CANONICAL_QUESTION_CONCEPT_ID").isNotNull() &
        F.col("POWERFORM_QUESTION_CONCEPT_ID").isNotNull() &
        (F.col("CANONICAL_QUESTION_CONCEPT_ID")!=F.col("POWERFORM_QUESTION_CONCEPT_ID")))
      .withColumn("VALUE_MAPPING_CONFLICT_IND",
        F.col("CANONICAL_VALUE_CONCEPT_ID").isNotNull() &
        F.col("POWERFORM_VALUE_CONCEPT_ID").isNotNull() &
        (F.col("CANONICAL_VALUE_CONCEPT_ID")!=F.col("POWERFORM_VALUE_CONCEPT_ID")))
      .withColumn("UNIT_MAPPING_CONFLICT_IND",
        F.col("CANONICAL_UNIT_CONCEPT_ID").isNotNull() &
        F.col("POWERFORM_UNIT_CONCEPT_ID").isNotNull() &
        (F.col("CANONICAL_UNIT_CONCEPT_ID")!=F.col("POWERFORM_UNIT_CONCEPT_ID")))
      .withColumn("QUESTION_CONCEPT_ID",F.coalesce("CANONICAL_QUESTION_CONCEPT_ID","POWERFORM_QUESTION_CONCEPT_ID"))
      .withColumn("VALUE_CONCEPT_ID",F.coalesce("CANONICAL_VALUE_CONCEPT_ID","POWERFORM_VALUE_CONCEPT_ID"))
      .withColumn("UNIT_CONCEPT_ID",F.coalesce("CANONICAL_UNIT_CONCEPT_ID","POWERFORM_UNIT_CONCEPT_ID"))
      .withColumn("QUESTION_MAPPING_SOURCE",
        F.when(F.col("CANONICAL_QUESTION_CONCEPT_ID").isNotNull(),"CANONICAL")
         .when(F.col("POWERFORM_QUESTION_CONCEPT_ID").isNotNull(),F.lit("POWERFORM_S5")))
      .withColumn("VALUE_MAPPING_SOURCE",
        F.when(F.col("CANONICAL_VALUE_CONCEPT_ID").isNotNull(),"CANONICAL")
         .when(F.col("POWERFORM_VALUE_CONCEPT_ID").isNotNull(),F.lit("POWERFORM_S5")))
      .withColumn("UNIT_MAPPING_SOURCE",
        F.when(F.col("CANONICAL_UNIT_CONCEPT_ID").isNotNull(),"CANONICAL")
         .when(F.col("POWERFORM_UNIT_CONCEPT_ID").isNotNull(),F.lit("POWERFORM_S5"))))

def assert_canonical_no_regression(before_df,after_df,key_cols):
    b=(before_df.where(F.col("QUESTION_MAPPING_SOURCE")=="CANONICAL")
       .select(*key_cols,F.col("QUESTION_CONCEPT_ID").alias("_BQ"),F.col("VALUE_CONCEPT_ID").alias("_BV"))
       .dropDuplicates(key_cols))
    a=after_df.select(*key_cols,F.col("QUESTION_CONCEPT_ID").alias("_AQ"),F.col("VALUE_CONCEPT_ID").alias("_AV"))
    bad=(b.join(a,key_cols,"inner").where(
      (~F.col("_BQ").eqNullSafe(F.col("_AQ"))) | (~F.col("_BV").eqNullSafe(F.col("_AV")))).count())
    assert bad==0, f"G-C6-3 canonical regressions: {bad}"
# S5-POWERFORM END
...
def apply_contextual_mappings(
    ...
    # S5-POWERFORM BEGIN — canonical values win; S5 fills nulls only.
    output = apply_s5_powerform_precedence(output)
    # S5-POWERFORM END
...
def changed_lane_candidates(
    ...
    keys = list(LANE_KEY_CONTRACTS[table_name])
    # Compare exactly the fields consumed by each canonical lane projection.
    # Publisher timestamps, trigger labels, row hashes and other provenance must not
    # turn a no-op publisher rewrite into hundreds of millions of PowerForm candidates.
    projection_columns = {
        SRC_NOMEN: (
            "NOMENCLATURE_ID", "SOURCE_ROW_KEY", "SOURCE_DELETED_IND",
            "OMOP_MANUAL_CONCEPT_ID", "OMOP_MANUAL_VALUE_CONCEPT_ID",
            "OMOP_MANUAL_VALUE_RULE_KEY", "OMOP_CONCEPT_ID", "OMOP_MATCH_NUMBER",
            "OMOP_STANDARD_CONCEPT", "OMOP_MANUAL_RULE_KEY",
            "OMOP_MANUAL_AMBIGUOUS_IND", "OMOP_MANUAL_VALUE_AMBIGUOUS_IND",
            "OMOP_MANUAL_CANDIDATE_CONCEPT_IDS",
            "OMOP_MANUAL_VALUE_CANDIDATE_CONCEPT_IDS",
        ),
        SRC_CODED: (
            "NOMENCLATURE_ID", "RESULT_CD", "SOURCE_ROW_KEY",
            "CR_CURRENT_AT_LOAD_IND", "OMOP_MANUAL_CONCEPT",
            "OMOP_MANUAL_VALUE_CONCEPT", "OMOP_MAPPING_RULE_ID",
            "OMOP_MAPPING_AMBIGUOUS_IND", "OMOP_VALUE_MAPPING_AMBIGUOUS_IND",
            "OMOP_MAPPING_CANDIDATE_CONCEPT_IDS",
            "OMOP_VALUE_MAPPING_CANDIDATE_CONCEPT_IDS",
        ),
        SRC_NUMERIC: (
            "NUMERIC_RESULT", "STRING_RESULT_TEXT", "SOURCE_CURRENT_IND",
            "SOURCE_DELETED_IND", "OMOP_MANUAL_CONCEPT_ID",
            "OMOP_MANUAL_UNIT_CONCEPT_ID", "OMOP_MANUAL_UNITS",
            "OMOP_MAPPING_AMBIGUOUS_IND", "OMOP_UNIT_MAPPING_AMBIGUOUS_IND",
            "OMOP_MAPPING_VALID_IND", "OMOP_UNIT_MAPPING_VALID_IND",
            "OMOP_MAPPING_RULE_ID", "OMOP_UNIT_MAPPING_RULE_ID",
            "OMOP_MAPPING_CANDIDATE_CONCEPT_IDS",
            "OMOP_UNIT_MAPPING_CANDIDATE_CONCEPT_IDS",
        ),
        SRC_TEXT: (
            "TEXT_RESULT_SHORT", "TEXT_RESULT", "RESULT_DT_TM",
            "SOURCE_CURRENT_IND", "SOURCE_DELETED_IND",
            "OMOP_MANUAL_CONCEPT_ID", "OMOP_MANUAL_VALUE_CONCEPT_ID",
            "OMOP_MANUAL_UNIT_CONCEPT_ID", "OMOP_MANUAL_UNITS",
            "OMOP_MAPPING_AMBIGUOUS_IND", "OMOP_VALUE_MAPPING_AMBIGUOUS_IND",
            "OMOP_UNIT_MAPPING_AMBIGUOUS_IND", "OMOP_MAPPING_VALID_IND",
            "OMOP_VALUE_MAPPING_VALID_IND", "OMOP_UNIT_MAPPING_VALID_IND",
            "OMOP_MAPPING_RULE_ID", "OMOP_VALUE_MAPPING_RULE_ID",
            "OMOP_UNIT_MAPPING_RULE_ID", "OMOP_MAPPING_CANDIDATE_CONCEPT_IDS",
            "OMOP_VALUE_MAPPING_CANDIDATE_CONCEPT_IDS",
            "OMOP_UNIT_MAPPING_CANDIDATE_CONCEPT_IDS",
        ),
        SRC_DATE: ("RESULT_DT_TM", "SOURCE_CURRENT_IND", "SOURCE_DELETED_IND"),
    }
    semantic = [
        name for name in projection_columns[table_name]
        if name in changes.columns and name not in keys
    ]
    if not semantic:
        raise RuntimeError(f"No semantic CDF columns available for {table_name}")
    payload = F.sha2(
        F.to_json(
            F.struct(*[F.col(name).alias(name) for name in semantic]),
            {"ignoreNullFields": "false"},
        ),
        256,
    )
    hashed = changes.withColumn("_SEMANTIC_HASH", payload)
    commits = hashed.groupBy(*keys, "_commit_version").agg(
        F.max(F.when(
            F.col("_change_type").isin("update_preimage", "delete"),
            F.col("_SEMANTIC_HASH"),
        )).alias("_PRE_HASH"),
        F.max(F.when(
    ...
    meaningful = commits.where(~F.col("_PRE_HASH").eqNullSafe(F.col("_POST_HASH")))
    events = meaningful.select(F.col("EVENT_ID").cast("long").alias("EVENT_ID"))         .where(F.col("EVENT_ID").isNotNull()).distinct()
    if is_empty(events):
        return None
