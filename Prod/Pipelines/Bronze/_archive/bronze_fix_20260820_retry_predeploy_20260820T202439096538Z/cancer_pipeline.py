...
# S3-A11 BEGIN: ARIA pharmacy approval timestamp source contract
_assert_columns(SRC_ARIA_RX, ["pt_id", "pt_visit_id", "rx_id", "pharm_appr_tstamp", "ADC_UPDT"])
# S3-A11 END: ARIA pharmacy approval timestamp source contract
_assert_columns(SRC_ARIA_AGT, [
    ...
    # S3-A11 BEGIN: ARIA treatment-plan initiation timestamp source contract
    "set_date_tp_init",
    # S3-A11 END: ARIA treatment-plan initiation timestamp source contract
])
...
CANCER_PUBLICATION_LOGIC_VERSION = "2026.08.19.1"


def source_fingerprint(tables, schema):
    # Source versions alone are insufficient: an additive schema or mapping-logic
    # release must invalidate the old checkpoint even when all inputs are unchanged.
    return bronze_json({
        "source_versions": {
        ...
        "publication_contract": {
            "logic_version": CANCER_PUBLICATION_LOGIC_VERSION,
            "schema": [
                {
                    "name": field.name,
...
def publish(build_fn, target, schema, comment, key_cols, sources,
    ...
    fp = source_fingerprint(sources, schema)
...
# S4-C8 BEGIN: document protected reviewed non-drug decisions
TERM_MAP_COMMENT = (
    ...
    "'Maps to'; ambiguous -> NULL. Rows with mapping_status in ('OVERRIDE', "
    "'NOT_DRUG') are curated by hand and never updated by the pipeline."
)
# S4-C8 END: document protected reviewed non-drug decisions
...
SCHEMA_TERM_MAP = StructType([
    ...
    # S4-C8 BEGIN: document NOT_DRUG status
    StructField("mapping_status", StringType(), True, metadata={"comment": "EXACT_NAME / EXACT_SYNONYM / OVERRIDE / NOT_DRUG / AMBIGUOUS / UNMAPPED."}),
    # S4-C8 END: document NOT_DRUG status
...
def build_aria_arm() -> DataFrame:
    ...
    base = (
        ...
        .select(
            ...
            # S3-A11 BEGIN: RDE ARIA parity fields; full composite joins already enforced above
            F.col("arx.set_date_tp_init").cast("timestamp").alias("ARIA_SET_DATE_TP_INIT"),
            F.col("rx.pharm_appr_tstamp").cast("timestamp").alias("ARIA_PHARM_APPR_TSTAMP"),
            # ARIA_PT_INST_KEY_ID deliberately omitted: only 101 values and no deterministic
            # treatment-grain row identity from the AGT/RX composite.
            # S3-A11 END: RDE ARIA parity fields
            ...
            # S3-A11 BEGIN: header changes must participate in source provenance
            F.greatest(F.col("arx.ADC_UPDT"), F.col("rx.ADC_UPDT")).alias("SRC_ADC_UPDT"),
            # S3-A11 END: header changes participate in source provenance
        )
    )
...
def consolidate(aria_arm: DataFrame, iqemo_arm: DataFrame) -> DataFrame:
    ...
    matched = matched_pairs.select(
        ...
        # S3-A11 BEGIN: carry ARIA parity fields through matched consolidation
        F.col("ar_ARIA_SET_DATE_TP_INIT").alias("ARIA_SET_DATE_TP_INIT"),
        F.col("ar_ARIA_PHARM_APPR_TSTAMP").alias("ARIA_PHARM_APPR_TSTAMP"),
        # S3-A11 END: carry ARIA parity fields
...
CANCER_TREATMENT_COMMENT = (
    ...
    # S3-A11 BEGIN: parity/disposition comment
    "ARIA_SET_DATE_TP_INIT and ARIA_PHARM_APPR_TSTAMP are joined on the deterministic "
    "agent/header composites. ARIA_PT_INST_KEY_ID is omitted because no deterministic "
    "treatment-grain row identity exists in aria_pt_inst_key."
    # S3-A11 END: parity/disposition comment
)
...
SCHEMA_CANCER_TREATMENT = StructType([
    ...
    # S3-A11 BEGIN: additive ARIA parity columns
    _f("ARIA_SET_DATE_TP_INIT", TimestampType(), "ARIA treatment-plan initiation timestamp from aria_agt_rx.set_date_tp_init; 87.92% filled at the 2026-08-12 baseline."),
    _f("ARIA_PHARM_APPR_TSTAMP", TimestampType(), "ARIA pharmacy approval timestamp from aria_rx.pharm_appr_tstamp joined by (pt_id, pt_visit_id, rx_id); 72.10% filled at the 2026-08-12 baseline."),
    # ARIA_PT_INST_KEY_ID is intentionally omitted because aria_pt_inst_key has many rows per
    # patient and no deterministic treatment-grain join defined by the AGT/RX composite.
    # S3-A11 END: additive ARIA parity columns
...
results.append(publish(
    ...
    # S4-C8 BEGIN: preserve reviewed overrides and non-drug decisions
    protect_condition="t.mapping_status IN ('OVERRIDE', 'NOT_DRUG')",
    # S4-C8 END: preserve reviewed overrides and non-drug decisions
