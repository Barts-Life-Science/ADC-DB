...
def _recorded_source_version(target, source):
    if not spark.catalog.tableExists(target):
        return None
    properties = spark.sql(f"DESCRIBE DETAIL {target}").first()["properties"] or {}
    previous = {
...
def pinned_increment(source, target, pipeline):
    ...
    previous_version = _recorded_source_version(target, source)

    if previous_version is None:
        inc = snap
    elif previous_version > version:
        raise RuntimeError(
            f"{source}: recorded source version {previous_version} is ahead of "
        ...
        try:
            ...
            # Force CDF validation inside this fail-closed branch. Timestamp
            # watermarks miss newly landed rows carrying historical ADC_UPDT.
            _ = inc.schema
            inc.limit(1).collect()
        except Exception as exc:
            raise RuntimeError(
                f"{source}: CDF unavailable for versions "
                f"{previous_version + 1}-{version}; a guarded full rebuild is required"
            ) from exc
...
def transform(source, target, pipeline, pk_col, entity_col, type_col, type_map):
    snap, inc, boundary, old_wm, version = pinned_increment(source, target, pipeline)
