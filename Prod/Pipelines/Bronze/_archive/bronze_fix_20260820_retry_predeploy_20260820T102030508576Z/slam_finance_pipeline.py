...
def _ensure_present_column(table):
    if not table_exists(table):
        return
    if "SOURCE_PRESENT_IND" not in spark.table(table).columns:
        spark.sql(f"ALTER TABLE {table} ADD COLUMNS (SOURCE_PRESENT_IND BOOLEAN)")
        spark.sql(f"UPDATE {table} SET SOURCE_PRESENT_IND = true WHERE SOURCE_PRESENT_IND IS NULL")
...
def _reconcile_hrg_snapshot(target, source, target_key, source_key_expr):
    spark.sql(f"""
        ...
        WHEN MATCHED AND NOT COALESCE(t.SOURCE_PRESENT_IND, false) THEN UPDATE SET
            t.SOURCE_PRESENT_IND = true, t.ADC_UPDT = current_timestamp()
...
def _s6b_hrg_lifecycle_fixture():
    ...
    ).withColumn("ADC_UPDT", F.current_timestamp()).write.saveAsTable(target)
    spark.sql(f"DELETE FROM {source} WHERE K='B'")
    _reconcile_hrg_snapshot(target, source, "K", "K")
    assert spark.table(target).where("K='B'").collect()[0]["SOURCE_PRESENT_IND"] is False
    spark.sql(f"INSERT INTO {source} VALUES ('B')")
    _reconcile_hrg_snapshot(target, source, "K", "K")
    assert spark.table(target).where("K='B'").collect()[0]["SOURCE_PRESENT_IND"] is True
    spark.sql(f"DROP TABLE IF EXISTS {source}")
    spark.sql(f"DROP TABLE IF EXISTS {target}")
...
def _ensure_target_from_stage(target, stage):
    ...
    target_schema = [(f.name, f.dataType.simpleString()) for f in spark.table(target).schema.fields]
    stage_schema = [(f.name, f.dataType.simpleString()) for f in spark.table(stage).schema.fields]
    assert target_schema == stage_schema, (
        f"{target}: stage schema drift; target={target_schema}, stage={stage_schema}")

def _replace_validated_slice(target, stage, extract_cd):
    _ensure_target_from_stage(target, stage)
    before_other = spark.table(target).where(F.col("EXTRACT_CD") != extract_cd).count()
    keys = (
        ["EXTRACT_CD", "ACTIVITY_RECORD_ID", "LINE_HASH"]
        if "LINE_HASH" in spark.table(stage).columns
    ...
    spark.sql(f"""
        ...
        SELECT * FROM {publish}
    """)
    after_other = spark.table(target).where(F.col("EXTRACT_CD") != extract_cd).count()
    assert before_other == after_other, (
        f"{target}: REPLACE WHERE changed another extract ({before_other}->{after_other})")
    target_present = spark.table(target).where(
        (F.col("EXTRACT_CD") == extract_cd) & F.col("SOURCE_PRESENT_IND")
    )
    assert stage_df.count() == target_present.count(), f"{target}: present slice row-count mismatch"
