...
def needs_run(target: str, sources: list) -> bool:
    ...
    if target == TGT_TERM:
        doctrine_violation = (
            spark.table(target)
            .where(
                (F.col("SOURCE_PRESENT_IND") == F.lit(True))
                & (
                    F.col("PERSON_ID").isNotNull()
                    | ~F.col("PERSON_LINK_STATUS").eqNullSafe(
                        F.lit("EXAM_TABLE_NOT_LANDED")
                    )
                )
            )
            .limit(1)
        ...
        if doctrine_violation:
            print(
                "[ENDOBASE] map_endobase_exam_term contains legacy direct-patient "
                "linkage; forcing self-healing rebuild"
            )
            return True
    recorded_sources = {
        row["source_table"]
    ...
    if not bronze_table_exists(target):
        ...
        if target == TGT_TERM:
            matched_update_condition += (
                ...
                "OR NOT (t.PERSON_LINK_STATUS <=> 'EXAM_TABLE_NOT_LANDED')"
            )
        (
