...
if _drug_needed:
    ...
    _valid_vtms = (
        ...
        .select(F.col("concept_id").cast("long").alias("_valid_vtm_concept_id"))
    )
    _curated_valid_vtms = _valid_vtms.select(
        F.col("_valid_vtm_concept_id").alias("_curated_valid_vtm_concept_id")
    )
    _drug_mapping_validation = (
        spark.table(DRUG_TABLE)
        .where(F.col("SOURCE_PRESENT_IND"))
        .join(
            _curated_valid_vtms,
            F.col("CURATED_DMD_CONCEPT_ID").cast("long")
            == F.col("_curated_valid_vtm_concept_id"),
            "left",
        )
        .withColumn(
            ...
            F.when(
                ...
                & F.col("_curated_valid_vtm_concept_id").isNotNull(),
                F.col("_curated_valid_vtm_concept_id"),
            ).otherwise(F.col("DMD_VTM_CONCEPT_ID").cast("long")),
        )
    )
    _approved_non_vtm_fallbacks = _drug_mapping_validation.where(
        (F.col("CURATED_STATUS") == "APPROVED")
        & F.col("CURATED_DMD_CONCEPT_ID").isNotNull()
        & F.col("_curated_valid_vtm_concept_id").isNull()
    ).count()
    if _approved_non_vtm_fallbacks:
        print(
            "[JAC] INFO: ignored approved curated non-VTM dm+d overrides and "
            f"used automatic VTM fallback: {_approved_non_vtm_fallbacks}"
        )
    ...
    _bad_drug_mappings = (
        ...
        .join(
            _valid_vtms,
            F.col("_effective_concept_id") == F.col("_valid_vtm_concept_id"),
            "left_anti",
        )
        .count()
    )
    assert (
        ...
        f"Invalid effective dm+d VTM mappings in {DRUG_TABLE}: "
        f"{_bad_drug_mappings}"
    )
