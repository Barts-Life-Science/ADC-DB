...
def build_rtt_pathway(xwalk: DataFrame, lookup: DataFrame) -> tuple[DataFrame, dict]:
    ...
    # SeqNoDESC is source orientation metadata, not a key. The August 19
    # landing contains distinct PeriodOID rows sharing orientation values,
    # including duplicate source "latest" markers. Preserve every period and
    # derive one deterministic latest row per pathway.
    duplicate_orientation_groups = (
        period.groupBy("SOURCE_SYSTEM_OID", "PATHWAY_OID", "SEQ_NO_DESC")
        .count()
        .where(F.col("count") > 1)
        .count()
    )
    source_latest_marker_anomalies = (
        period.groupBy("SOURCE_SYSTEM_OID", "PATHWAY_OID")
        .agg(F.sum((F.col("SEQ_NO_DESC") == 1).cast("long")).alias("latest_rows"))
        .where(F.col("latest_rows") != 1)
        .count()
    )
    period_has_clock_data = (
        F.col("START_DATETIME").isNotNull()
        | F.col("STOP_DATETIME").isNotNull()
        | F.col("START_RTT_ACTIVITY_OID").isNotNull()
        | F.col("STOP_RTT_ACTIVITY_OID").isNotNull()
    ).cast("int")
    latest_period_window = Window.partitionBy(
        "SOURCE_SYSTEM_OID", "PATHWAY_OID"
    ).orderBy(
        F.when(F.col("SEQ_NO_DESC") == 1, F.lit(0)).otherwise(F.lit(1)).asc(),
        F.col("SEQ_NO_DESC").asc_nulls_last(),
        period_has_clock_data.desc(),
        F.col("PERIOD_MODIFIED_DATETIME").desc_nulls_last(),
        F.col("PERIOD_CREATED_DATETIME").desc_nulls_last(),
        F.col("PERIOD_SOURCE_ADC_UPDT").desc_nulls_last(),
        F.col("PERIOD_OID").desc_nulls_last(),
    )
    period = period.withColumn(
        "__DERIVED_LATEST_PERIOD_RN",
        F.row_number().over(latest_period_window),
    )
    ...
    discrepancy = (
        (F.col("__DERIVED_LATEST_PERIOD_RN") == 1)
        & (
    ...
    clocked = (
        ...
        .drop(
            "__CORE_CLOCK_START_DT_TM",
            "__CORE_CLOCK_STOP_DT_TM",
            "__CORE_MATCH",
            "__DERIVED_LATEST_PERIOD_RN",
        )
    )
    ...
    return hybrid, {
        ...
        "duplicate_orientation_groups_retained": int(duplicate_orientation_groups),
        "source_latest_marker_anomaly_pathways": int(source_latest_marker_anomalies),
        "period_type_safety": period_type_gate,
        "exactly_one_latest_per_pathway_source_gate": False,
        "exactly_one_latest_per_pathway_derived": True,
        "reference_no_fanout": True,
    }
