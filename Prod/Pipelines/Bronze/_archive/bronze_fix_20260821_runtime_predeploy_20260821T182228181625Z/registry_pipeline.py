...
AUDIT_COLS = {"ROW_HASH", "PIPELINE_LOADED_AT", "IS_CURRENT_IN_SOURCE", "SOURCE_PRESENT_IND", "SOURCE_ABSENT_DETECTED_TS"}
...
def drop_pii(df: DataFrame, extra=()):
    """Compatibility shim: retain identifiers in Bronze and report what older code removed."""
    retained = {name for name in df.columns if is_pii_column(name)}
    retained.update(name for name in extra if name in df.columns)
    return df, sorted(retained)
...
def to_snake(df: DataFrame) -> DataFrame:
    renames = {name: snake_name(name) for name in df.columns}
    ...
    return df.select(
        *[
            F.col(quote + name.replace(quote, quote + quote) + quote).alias(renames[name])
    ...
    if not table_exists(target):
        ...
        (
            ...
            .whenNotMatchedBySourceUpdate(
                ...
                set={
                    "IS_CURRENT_IN_SOURCE": "false",
                    "SOURCE_PRESENT_IND": "false",
                    "SOURCE_ABSENT_DETECTED_TS": "COALESCE(t.SOURCE_ABSENT_DETECTED_TS, current_timestamp())",
...
def _registry_tag_tables():
    for table_name in REGISTRY_TABLE_KEYS:
        ...
        for col_name in spark.table(table).columns:
            ...
            if (
                is_pii_column(col_name)
                or upper in {"MRN", "PATIENTID", "NHS_NUMBER", "NHSNUMBER"}
...
# Retain complete PRE1 fields at top level for compatibility. Preserve every field
# from the other source modules in typed structs so the table remains below Unity
# Catalog's 500-column/two-tag boundary without discarding identifiers or clinical data.
base_raw = spark.table(f"{IWEB}.reg_cs2010g_pre1")
base_linked = resolve_persons(base_raw, mrn_col="MRN", nhs_col="NHSNumber")
module_schemas = {"PRE1": base_raw.schema}
module_output_names = {
    "PRE1": {
        name: ("PRE1_ADC_UPDT" if name == "ADC_UPDT" else name)
        for name in base_raw.columns
    }
}
...
for prefix, table_name in MODULES[1:]:
    module_raw = spark.table(f"{IWEB}.{table_name}")
    module_schemas[prefix] = module_raw.schema
    module = module_raw
    output_names = {}
    if "ADC_UPDT" in module.columns:
        module = module.withColumnRenamed("ADC_UPDT", f"{prefix}_ADC_UPDT")
    for raw_name in module_raw.columns:
        current_name = f"{prefix}_ADC_UPDT" if raw_name == "ADC_UPDT" else raw_name
        output_name = (
            f"{prefix}_{current_name}"
            if current_name != "EntryId" and current_name in kept
            else current_name
        )
        output_names[raw_name] = output_name
    module = module.select(
        *[
            F.col(
                f"{prefix}_ADC_UPDT" if name == "ADC_UPDT" else name
            ).alias(output_names[name])
            for name in module_raw.columns
        ]
    )
    module_output_names[prefix] = output_names
    kept.update(output_names.values())
...
episode_wide = (
    merged.withColumn(
        ...
        F.greatest(*[F.col(name) for name in last_changed_columns]),
    )
    .withColumn("ADC_UPDT", F.greatest(*[F.col(name) for name in adc_columns]))
)
episode_wide = with_source_metadata(
    episode_wide,
    "reg_cs2010g_pre1+reg_cs2010g_pre2+reg_cs2010g_post1+reg_cs2010g_post2",
    "EntryId",
)
episode_wide = to_snake(episode_wide)

module_output_names = {
    prefix: {
        snake_name(raw_name): snake_name(output_name)
        for raw_name, output_name in names.items()
    }
    for prefix, names in module_output_names.items()
}
PROMOTED_MODULE_FIELDS = {"DATE_LAST_CHANGED", "ADC_UPDT"}
MODULE_STRUCT_NAMES = {
    "PRE2": "PRE2_MODULE",
    "POST1": "POST1_MODULE",
    "POST2": "POST2_MODULE",
...
def _module_struct_expression(frame: DataFrame, prefix: str):
    available = set(frame.columns)
    expressions = []
    for field in module_schemas[prefix].fields:
        raw_name = snake_name(field.name)
        if raw_name == "ENTRY_ID" or raw_name in PROMOTED_MODULE_FIELDS:
            continue
        source_name = module_output_names[prefix][raw_name]
        value = (
            F.col(source_name).cast(field.dataType)
            if source_name in available
            else F.lit(None).cast(field.dataType)
        )
        expressions.append(value.alias(raw_name))
    return F.struct(*expressions).alias(MODULE_STRUCT_NAMES[prefix])


moved_columns = {
    module_output_names[prefix][snake_name(field.name)]
    for prefix in MODULE_STRUCT_NAMES
    for field in module_schemas[prefix].fields
    if snake_name(field.name) not in {"ENTRY_ID", *PROMOTED_MODULE_FIELDS}
}
episode = episode_wide.select(
    *[F.col(name) for name in episode_wide.columns if name not in moved_columns],
    *[_module_struct_expression(episode_wide, prefix) for prefix in MODULE_STRUCT_NAMES],
)
assert len(episode.columns) <= 500, (
    f"Cardiac surgery episode has {len(episode.columns)} top-level columns; "
    "two mandatory IG tags require at most 500."
)
for prefix, struct_name in MODULE_STRUCT_NAMES.items():
    expected = {
        snake_name(field.name)
        for field in module_schemas[prefix].fields
        if snake_name(field.name) not in {"ENTRY_ID", *PROMOTED_MODULE_FIELDS}
    }
    actual = set(episode.schema[struct_name].dataType.fieldNames())
    assert actual == expected, (prefix, sorted(expected - actual), sorted(actual - expected))


def _episode_business_from_legacy(prior: DataFrame) -> DataFrame:
    available = set(prior.columns)
    expressions = []
    for field in episode.schema.fields:
        if field.name in MODULE_STRUCT_NAMES.values():
            prefix = next(
                key for key, value in MODULE_STRUCT_NAMES.items() if value == field.name
            )
            expressions.append(_module_struct_expression(prior, prefix))
    ...
    return prior.select(*expressions)


def _episode_source_frame(frame: DataFrame) -> DataFrame:
    return (
        with_row_hash(frame)
        .withColumn("PIPELINE_LOADED_AT", F.current_timestamp())
        .withColumn("IS_CURRENT_IN_SOURCE", F.lit(True))
...
def _migrate_episode_schema_if_required(target: str, fresh_business: DataFrame) -> None:
    if not table_exists(target):
        return
    prior = spark.table(target)
    expected_columns = set(_episode_source_frame(fresh_business).columns)
    if len(prior.columns) <= 500 and set(prior.columns) == expected_columns:
        return

    prior_version = int(table_version(target))
    prior_absent = prior.where(
        ~F.coalesce(F.col("SOURCE_PRESENT_IND"), F.col("IS_CURRENT_IN_SOURCE"), F.lit(True))
    )
    old_loaded = (
        F.col("PIPELINE_LOADED_AT")
        if "PIPELINE_LOADED_AT" in prior.columns
        else F.lit(None).cast("timestamp")
    )
    old_absent = (
        F.col("SOURCE_ABSENT_DETECTED_TS")
        if "SOURCE_ABSENT_DETECTED_TS" in prior.columns
        else F.lit(None).cast("timestamp")
    )
    prior_audit = prior_absent.select(
        "ENTRY_ID",
        old_loaded.alias("_OLD_PIPELINE_LOADED_AT"),
        old_absent.alias("_OLD_SOURCE_ABSENT_DETECTED_TS"),
    )
    legacy_business = _episode_business_from_legacy(prior_absent)
    legacy_absent = (
        with_row_hash(legacy_business)
        .join(prior_audit, "ENTRY_ID", "left")
        .withColumn(
            "PIPELINE_LOADED_AT",
            F.coalesce(F.col("_OLD_PIPELINE_LOADED_AT"), F.current_timestamp()),
        )
        .withColumn("IS_CURRENT_IN_SOURCE", F.lit(False))
        .withColumn("SOURCE_PRESENT_IND", F.lit(False))
        .withColumn(
            "SOURCE_ABSENT_DETECTED_TS",
            F.coalesce(
                F.col("_OLD_SOURCE_ABSENT_DETECTED_TS"),
                F.current_timestamp(),
            ),
        )
        .drop("_OLD_PIPELINE_LOADED_AT", "_OLD_SOURCE_ABSENT_DETECTED_TS")
    )
    migration_stage = target + "__compact_migration_absent"
    spark.sql(f"DROP TABLE IF EXISTS {qname(migration_stage)}")
    (
        legacy_absent.write.format("delta")
        .mode("overwrite")
        .option("overwriteSchema", "true")
        .saveAsTable(migration_stage)
    )
    retained_absent_rows = spark.table(migration_stage).count()
    migrated = _episode_source_frame(fresh_business).unionByName(
        spark.table(migration_stage),
        allowMissingColumns=False,
    )
    verify_unique_key(migrated, ["ENTRY_ID"])
    (
        ...
        + json.dumps(
            {
                ...
                "prior_columns": len(prior.columns),
                "new_columns": len(migrated.columns),
                "retained_absent_rows": retained_absent_rows,
            },
            sort_keys=True,
        )
    )
...
episode_target = f"{TARGET_SCHEMA}.iweb_cardiac_surgery_episode"
_migrate_episode_schema_if_required(episode_target, episode)

registry_update_table(
    episode,
    episode_target,
    ["ENTRY_ID"],
    "iWeb CS2010G adult cardiac surgery registry. Complete PRE1 fields remain top-level; "
    "PRE2, POST1 and POST2 source fields are retained losslessly in typed module structs. "
    "This preserves identifiers and clinical content while remaining within Unity Catalog's "
    "1,000-tag-per-table quota.",
    {
...
print("[IDENTIFIER_RETAIN] cardiac_surgery_procedure " + json.dumps(procedure_drops))
...
print("[IDENTIFIER_RETAIN] cardiac_surgery_followup " + json.dumps(followup_drops))
...
def build_single_registry(
    ...
    print(f"[IDENTIFIER_RETAIN] {target_table} " + json.dumps(dropped))
...
def load_mdt(source_table: str, registry_type: str) -> DataFrame:
    ...
    print(f"[IDENTIFIER_RETAIN] {source_table} " + json.dumps(dropped))
...
print("[IDENTIFIER_RETAIN] mortality_review " + json.dumps(mortality_drops))
...
registry_update_table(
    ...
    "flags. Patient names, contact details and next-of-kin content are retained and governed with IG tags.",
    {
...
# Bronze retains source identifiers; assert that source identifier columns remain published.
IDENTIFIER_SOURCE_MAP = {
    "mediconnect_device": [f"{RAW}.mediconnect_mc_devices"],
    "iweb_cardiac_surgery_episode": [f"{IWEB}.reg_cs2010g_pre1"],
    "iweb_cardiac_surgery_procedure": [f"{IWEB}.reg_cs2010g_subprocedure"],
    "iweb_cardiac_surgery_followup": [f"{IWEB}.reg_cs2010g_followup"],
    "iweb_coronary_lesion": [f"{IWEB}.reg_coronary_subprocedure"],
    "iweb_noncoronary_procedure": [f"{IWEB}.reg_noncoronary"],
    "iweb_acs_transfer": [f"{IWEB}.reg_dghminap"],
    "iweb_eracs_episode": [f"{IWEB}.reg_eracs"],
    "iweb_cardiac_mdt": [f"{IWEB}.reg_mdt", f"{IWEB}.reg_ctmdt"],
    "iweb_cardiac_mortality_review": [f"{IWEB}.reg_mort"],
}
missing_identifiers = []
for table_name, source_names in IDENTIFIER_SOURCE_MAP.items():
    available = set(spark.table(f"{TARGET_SCHEMA}.{table_name}").columns)
    expected = set()
    for source_name in source_names:
        for column in spark.table(source_name).columns:
            normal = _normalised_column_name(column)
            if is_pii_column(column) or normal in {"mrn", "nhsnumber", "patientid"}:
                expected.add(snake_name(column))
    missing_identifiers.extend(
        f"{table_name}.{column}" for column in sorted(expected - available)
    )
check(
    "identifiers_retained",
    not missing_identifiers,
    metric_value=len(missing_identifiers),
    detail=missing_identifiers,
)
