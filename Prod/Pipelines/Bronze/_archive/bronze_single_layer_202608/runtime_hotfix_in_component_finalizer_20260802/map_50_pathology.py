...
MP_FULL_BUILD_MANIFEST = f'{MP_TARGET}__full_build_manifest'
MP_FULL_BUILD_PROGRESS = f'{MP_TARGET}__full_build_progress'
...
def _max_timestamp_at_version(table_name: str, column_name: str | None, version: int) -> datetime | None:
    if column_name is None:
        return None
    row = spark.read.format('delta').option('versionAsOf', int(version)).table(table_name).select(F.max(F.col(column_name)).alias('max_ts')).first()
    return row['max_ts'] if row else None
...
def _install_pinned_full_build_views(manifest: dict) -> tuple[dict, list[str]]:
    ...
    for global_name, source_name in FULL_BUILD_GLOBAL_SOURCES.items():
        ...
        spark.read.format('delta').option('versionAsOf', version).table(table_name).createOrReplaceTempView(view_name)
...
def _read_changes(source_name: str, state: dict[str, dict], cutoffs: dict[str, dict]) -> tuple[DataFrame, str]:
    ...
    try:
        ...
        print(f'[map_pathology_v3] CDF fallback for {table_name}: {str(exc).splitlines()[0]}')
...
def _full_stage_table(build_id: str, suffix: str) -> str:
    ...
    return f'{MP_TARGET}__build_{safe_build}_{suffix}'
