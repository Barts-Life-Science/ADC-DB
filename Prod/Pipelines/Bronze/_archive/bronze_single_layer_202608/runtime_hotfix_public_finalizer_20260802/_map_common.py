...
# Capture Databricks' injected session in a notebook-owned name. This survives
# the %run module boundary even if the injected `spark` name is later absent from
# the helper function's globals.
...
def _pipeline_optional_bool_parameter(
    ...
    try:
        return str(dbutils.widgets.get(name)).strip().lower() in {
            "1",
            "true",
            "yes",
            "y",
...
def _pipeline_post_deployment_checks(
    ...
    try:
        for contract_result in bronze_assert_all_primary_contracts(prefix):
            results.append({
