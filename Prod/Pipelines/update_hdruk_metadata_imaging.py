...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE 1_inland.evan_demo.hdruk_metadata_col_imaging(
...
# MAGIC )
# MAGIC INSERT INTO 1_inland.evan_demo.hdruk_metadata_col_imaging
# MAGIC SELECT 
...
# MAGIC     AND c.table_schema = 'pacs'
# MAGIC     --AND c.table_name ILIKE 'omop_%'
# MAGIC   ) 
...
# MAGIC %sql
# MAGIC
# MAGIC MERGE INTO 1_inland.evan_demo.hdruk_metadata_col_imaging AS target
# MAGIC USING (
# MAGIC   SELECT 
# MAGIC     table_name,
# MAGIC     column_name, 
...
# MAGIC       AND table_schema = 'pacs'
# MAGIC       --AND table_name ILIKE 'omop_%'
# MAGIC     ) 
# MAGIC ) AS source
...
# MAGIC %sql
# MAGIC
# MAGIC CREATE OR REPLACE TABLE 1_inland.evan_demo.hdruk_metadata_tab_imaging(
...
# MAGIC %sql
# MAGIC INSERT INTO 1_inland.evan_demo.hdruk_metadata_tab_imaging
# MAGIC SELECT
# MAGIC   c.table_name,
# MAGIC   --CONCAT('[', array_join(collect_set(c.json_str), ','), ']'),
# MAGIC   collect_set(c.json_struct),
# MAGIC   NULL
# MAGIC FROM 1_inland.evan_demo.hdruk_metadata_col_imaging AS c
...
# MAGIC %sql
# MAGIC
# MAGIC MERGE INTO 1_inland.evan_demo.hdruk_metadata_tab_imaging AS target
# MAGIC USING (
...
api_path = "https://api.dev.hdruk.cloud/api/v1/integrations/datasets/1491"
headers = {
...
response = requests.get(
    "https://api.healthdatagateway.org/api/v1/datasets/1491",
    headers=headers
)
print(response)
...
'''
{
    ...
    "summary": {
        "title": "Barts Imaging Dataset",
        "abstract": "Barts Health NHS Trust Imaging Metadata and Report Dataset",
...
api_path = "https://api.healthdatagateway.org/api/v1/integrations/datasets/1491"
headers = {
    "x-application-id": dbutils.secrets.get(scope="adc_store", key="hdruk_app_id"),
    "x-client-id": dbutils.secrets.get(scope="adc_store", key="hdruk_client_id"),
    "Content-Type": "application/json"
