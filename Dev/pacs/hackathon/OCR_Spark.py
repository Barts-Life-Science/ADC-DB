...
def process_images_old(input_folder, output_folder, preprocessing, min_text_length, padding):
...
schema = StructType([
    ...
    StructField("coord_x_start", IntegerType()),
    StructField("coord_y_start", IntegerType()),
    StructField("coord_x_end", IntegerType()),
    StructField("coord_y_end", IntegerType()),
    StructField("detected_text", StringType()),
    StructField("detection_score", FloatType())
])
...
def detect_text_flat(iterator):
    import os
    os.environ["PADDLEX_HOME"] = "/local_disk0/paddlex"
    os.environ["TMPDIR"] = "/local_disk0/tmp"

    # Initialize once per partition
    ...
    for pdf in iterator:
        ...
        for _, row in pdf.iterrows():
            ...
            try:
                ...
                if results is not None:
                    for line in results:
                        ...
                        for word_info in line:
                            ...
                            # Convert polygon → axis-aligned bounding box
                ...
                # Optional: ensure at least one row per file
                if not results:
                    output_rows.append({
                        "path": path,
                        "coord_x_min": None,
...
def process_images(input_folder, output_folder, preprocessing, min_text_length, padding):
    ...
    print(df.count(), "files loaded into df.")
    #display(df)

    # Apply mapInPandas
    redacted_df =  df.mapInPandas(detect_text_flat, schema)
    display(redacted_df)
    return redacted_df
