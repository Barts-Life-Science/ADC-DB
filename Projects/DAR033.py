...
FROM (
    -- People seen in Dental Hospital
    SELECT 
    ...
    -- People in COCR0021 trial
    SELECT 
        DISTINCT reg.PERSON_ID AS PERSON_ID
    FROM 4_prod.raw.mill_pt_prot_reg reg
    JOIN 4_prod.raw.mill_prot_master mast 
        ON reg.PROT_MASTER_ID = mast.PROT_MASTER_ID
    WHERE reg.REMOVAL_REASON_CD = 0
    AND mast.PRIMARY_MNEMONIC_KEY = 'COCR0021'
) combined
"""
